...
#assigning bids and joker card count to new hand values
hand_to_bid = {}
...
# segregating hands with joker cards
for i in hands:
  if "@" in i:
    if len(set(i)) in [1,2]:
      five_oak.append(i)
      ...
      lts = list(set(i))
      lts.remove("@")
      if i.count(lts[0])==2 and i.count(lts[1])==2:
        full_house.append(i)
      else:
        four_oak.append(i)
    elif len(set(i))==4:
...
# segregating hands without joker cards
for i in hands:
  if "@" not in i:
    if len(set(i))==5:
      high_card.append(i)
